#! /usr/bin/env python3
# Autor @RemovFaller
# Więcej znajdziesz na stronie http://remofaller.xaa.pl
# Ostatnia Aktualizacja: 22/04/2022 - version 1.2
       ...
       ,rwKKKKKKKKKKKKKPv;,:'-':,;vPKKKKKKKKKKKKKwr,
          !KKKKKKKKKKKKKKK/            !KKKKKKKKKKKKKKK!
           !KKKKKKKKKKKKKKf              CKKKKKKKKKKKKKK!
            ...
            !KKKKKKKKKKKKK>"     KKkKKKKKKKKKKKKK!
              !KKKKKKKw;,h'-      WKK,"wKKKKKKK!
              !KKKKKKKKhi*;"             *ihKKKKKKKK!
            !KKKKKKKKKKKKK;                ;KKKKKKKKKKKKK!
           !KKKKKKKKKKKKK2>'     >2KKKKKKKKKKKKK!
          !KKKKKKKKKKKKKKKZ    KZKKKKKKKKKKKKKKK!
          !KKKKKKKKKKKKKKK5    KKKKKKKKKKKKKKK!
           !KKKKKKKKKKKqC;-    KCqKKKKKKKKKKK!
               <KKKKKKKKkr,      ,rSKKKKKKKK<
                      -"v]qj;- -;jq]v"-
                       {w}[ SpEcNaZ ]{b}
           {d}Skrypt do uzyskiwania informacji{w}
               {d}Autor {w}{r}@RemovFaller{w}"""

def menu():
    os.system("clear")
    print(logo)
    print(f"""
         {W}\033[2;30m Wybierz numer zadania lub wpisz EXIT aby wyjść {w}
    
        {w}{b}  01{w} Userrecon    {d} Wyszukiwanie nazw Użytkownika 
        {w}{b}  02{w} Facedumper   {d} Zrzuć informacje z Facebooka
        {w}{b}  03{w} Mailfinder   {d} Znajdź email z nazwą
        {w}{b}  04{w} Godorker     {d} Użyj wyszukiwarki Google
        {w}{b}  05{w} Phoneinfo    {d} Informacje o numerze telefonu
...
def iplocation():
    ...
    print(w+lines)
    getpass(space+"Wybierz ENTER aby powrócić ")
    menu()

def infoga(opt):
    x = str(input(f"{space}{b}>{w} Wpisz Domenę lub IP:{b} "))
    if not x: menu()
...
def phoneinfo():
    ...
    print(f"{space}{B} ZAKOŃCZONO {R} {no} {w}")
    try: print(f"{space}{b}-{w} Typ    : {y}{res['phone_type']}{w}")
    except KeyError: pass
    try: print(f"{space}{b}-{w} Prefiks  : {y}{res['country_prefix']}{w}")
    except KeyError: pass
    try: print(f"{space}{b}-{w} Kod    : {y}{res['country_code']}{w}")
    except KeyError: pass
    try: print(f"{space}{b}-{w} Kraj : {y}{res['country']}{w}")
    except KeyError: pass
    try: print(f"{space}{b}-{w} Międzynarodowy  : {y}{res['international_number']}{w}")
    except KeyError: pass
    try: print(f"{space}{b}-{w} Lokalny   : {y}{res['local_number']}{w}")
    except KeyError: pass
    try: print(f"{space}{b}-{w} Usługodawca: {y}{res['carrier']}{w}")
    except KeyError: pass
    print(w+lines)
    getpass(space+"Wybierz ENTER aby powrócić ")
    menu()
...
def godorker():
    ...
    for u in url:
        try:
            ...
            f.write(u+"\n")
            print(f"{space}{B} WYSZUKANO {w} {str(title)}\n{space}{d}{u}{w}")
...
def userrecon():
    username = str(input(f"{space}{w}{b}>{w} Wpisz nazwę Użytkownika:{b} ").lower())
